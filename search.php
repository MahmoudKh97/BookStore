		...
		<div class="page-content" >
			...
			<div>
				<div class="btHeading bt-md" >
					...
					<div class="top-heading"><?php echo ' " '.$search . ' "'?></div>
				</div>
				...
				<div class="container pad-top-50">
					...
					$query = "SELECT *  FROM `products` WHERE `NAME` LIKE '%$search%' AND `STATUS`='active' ORDER BY `ID` DESC";
					$result = $link->query($query);
					if ($result->num_rows == 0) {
					$query = "SELECT `products`.*  FROM `products`,`category` WHERE `category`.`CATEGORY` LIKE '%$search%' 
						AND `products`.`CATEGORY`=`category`.`ID` AND `products`.`STATUS`='active' ORDER BY `ID` DESC";
					$result = $link->query($query);
					}if ($result->num_rows == 0) {
					$query = "SELECT `products`.*  FROM `products`,`sub_category` WHERE `sub_category`.`SUB_CATEGORY` LIKE '%$search%' 
						AND `products`.`SUB_CATEGORY`=`sub_category`.`ID`AND `products`.`STATUS`='active' ORDER BY `ID` DESC";
					$result = $link->query($query);	
					}if ($result->num_rows > 0) {
					...
					';}	
						...
						echo '<h4 class="fancy-heading text-center">No results found ...</h4>';
					}
					?>	
					</div>
				</div>
